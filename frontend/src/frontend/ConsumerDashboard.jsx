...
const ConsumerDashboard = () => {
  ...
  const [stats, setStats] = useState({ monthlyUsage: 0, purchased: 0, savings: 0, walletSpent: 0 })
    ...
    const load = async () => {
      try {
        ...
        if (r.ok) {
          ...
          if (d.success) {
            ...
            const kwhBought = bought.reduce((sum, t) => sum + (Number(t.energyKwh) || 0), 0)
            const spent = bought.reduce((sum, t) => sum + (Number(t.amount) || 0), 0)
            const savings = Math.max(0, kwhBought * 0.15 - spent * 0.05)
            const sellers = new Set(bought.map((t) => t.counterparty)).size

            setStats((prev) => ({ ...prev, purchased: kwhBought, savings, walletSpent: spent }))
            setProfile((prev) => ({ ...prev, sellers }))
          }
        }
      } catch {}
      ...
      try {
        ...
        if (r.ok) {
          ...
          setStats((prev) => ({ ...prev, monthlyUsage: d.energyUsage || 0 }))
          setProfile((prev) => ({
            ...prev,
      ...
      try {
        const r = await fetch(apiUrl("/dashboard/marketplace-opportunities"), { headers })
        if (r.ok) {
          const d = await r.json()
      ...
      try {
        const r = await fetch(apiUrl("/dashboard/site-forecast?hours=12"), { headers })
        if (r.ok) {
          const d = await r.json()
    ...
    setConsumptionData((prev) => {
      const next = [
        ...prev,
        {
          timestamp,
          Consumed: consumed,
          "P2P Price": energyPrice * 0.05,
          "Grid Price": gridRefRate,
        },
      ]
  ...
  const estimatedHourlyCost = (liveLoad / 1000) * p2pRate
  ...
  const carbonSaved = stats.purchased * 0.25
  const sparkBars = usageHistory.map((value, index) => ({ index, value }))
        ...
        <div style={{ background: "rgba(6,8,16,.97)", borderBottom: `1px solid ${C.border}`, padding: "0 20px", display: "flex", alignItems: "center", justifyContent: "space-between", height: 44, position: "sticky", top: 52, zIndex: 40 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
            ...
            <span style={{ fontSize: 10, color: C.text3 }}>
              Welcome, <span style={{ color: C.blue }}>{user?.user?.name || user?.name || "Consumer"}</span>
            </span>
        ...
        <div style={{ padding: 16, display: "grid", gridTemplateColumns: "240px 1fr 220px", gap: 12, alignItems: "start" }}>
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            <Card title="Carbon Impact" accent={C.green}>
              {[
                { label: "Annual CO2 (est)", value: `${profile.carbonFootprint.toFixed(0)} kg`, color: C.red },
            ...
            <Card title="Rate Forecast - Next 12h" badge={<Badge color={C.green}>P2P cheaper</Badge>}>
              <ResponsiveContainer width="100%" height={160}>
                <LineChart data={forecast} margin={{ top: 5, right: 8, left: -16, bottom: 0 }}>
                  ...
                  <YAxis tick={{ fill: C.text3, fontSize: 8 }} axisLine={false} tickLine={false} />
                  <Tooltip
                    contentStyle={{ background: C.bg2, borderColor: C.border, borderRadius: 4, fontSize: 11 }}
                    formatter={(value, name) => [`${Number(value).toFixed(4)} tok`, name]}
                  />
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {opportunities.length === 0 ? (
                  ...
                  <motion.div key={index} whileHover={{ x: 4 }}
                    ...
                    <span style={{ fontSize: 18 }}>{op.type === "sell" ? "SELL" : "BUY"}</span>
                ...
                <motion.button onClick={() => navigate("/marketplace")} whileHover={{ x: 4 }} whileTap={{ scale: 0.97 }}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            <Card title="My Profile" accent={C.border}>
              {[
                { label: "Location", value: profile.location, icon: <MapPinIcon size={14} color={C.blue} /> },
                { label: "Wallet", value: walletDisplay, icon: <WalletIcon size={14} color={C.purple} /> },
                { label: "Sellers", value: profile.sellers, icon: <UsersIcon size={14} color={C.green} /> },
                  ...
                  <>
                    <div style={{ textAlign: "center", padding: "12px 0" }}>
                      <p style={{ fontFamily: "'Syne',sans-serif", fontWeight: 800, fontSize: 28, color: better ? C.green : C.red }}>
                        {better ? "DOWN" : "UP"} {Math.abs(Number(pct))}%
                      </p>
                    ...
                    <div style={{ marginTop: 10, padding: "8px 10px", background: C.bg2, borderRadius: 4, border: `1px solid ${(better ? C.green : C.red) + "40"}` }}>
                      <p style={{ fontSize: 10, color: better ? C.green : C.red, lineHeight: 1.5 }}>
                        {better
                          ? "Great. Your load is below zone average and may be eligible for surplus opportunities."
