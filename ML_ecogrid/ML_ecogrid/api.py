...
# Assuming these utilities exist in your local ml_utils.py
...
print("Loading model...")
# Ensure these paths are correct relative to your app entry point
...
async def forecast(req: ForecastRequest):
    ...
    # Build lookback sequence from past weather
    now = pd.Timestamp.now()
    past_weather = weather_df[weather_df["timestamp"] <= now].tail(LOOKBACK)
    ...
    # feature_scaler expects (n_samples, n_features), so we scale then reshape
    X_scaled = feat_scaler.transform(X)
    current_inp = X_scaled.reshape(1, LOOKBACK, len(FEATURE_COLS))

    future_wx = weather_df[weather_df["timestamp"] > now].head(req.forecast_hours)
    future_rows = list(future_wx.iterrows())
    ...
    for step in range(req.forecast_hours):
        ...
        ts_obj = now + timedelta(hours=step + 1)
        ts_str = ts_obj.strftime("%Y-%m-%dT%H:%M")

        if step < len(future_rows):
            _, frow = future_rows[step]
            solar_f = approx_solar(float(frow.get("shortwave_radiation", 0)), float(frow.get("cloudcover", 0)), zone)
            wind_f = approx_wind(float(frow.get("windspeed_10m", 0)), zone)
            hydro_f = approx_hydro(frow["timestamp"].month, zone)
        else:
            solar_f, wind_f = 0, 5 # Fallback
            hydro_f = approx_hydro(ts_obj.month, zone)
    ...
    f0 = forecasts[0]
    renewable_mix = compute_renewable_mix(f0["solar_gw"], f0["wind_gw"], f0["hydro_gw"], f0["thermal_gw"])
    carbon_intensity = compute_carbon_intensity(f0["solar_gw"], f0["wind_gw"], f0["hydro_gw"], f0["thermal_gw"])

    latest_wx = past_weather.iloc[-1] if not past_weather.empty else {}
    weather_now = {
        "temperature": round(float(latest_wx.get("temperature_2m", 0)), 1),
        "windspeed": round(float(latest_wx.get("windspeed_10m", 0)), 1),
        "cloudcover": round(float(latest_wx.get("cloudcover", 0)), 1),
        "irradiance": round(float(latest_wx.get("shortwave_radiation", 0)), 1),
    }
...
async def zones_summary():
    ...
    for zone_name, meta in ZONE_META.items():
        try:
            weather_df = fetch_live_weather(meta["lat"], meta["lng"])
            latest = weather_df[weather_df["timestamp"] <= now].iloc[-1]
            
            solar = approx_solar(float(latest.get("shortwave_radiation", 0)), float(latest.get("cloudcover", 0)), zone_name)
            wind = approx_wind(float(latest.get("windspeed_10m", 0)), zone_name)
            hydro = approx_hydro(latest["timestamp"].month, zone_name)
            demand = approx_demand(float(latest.get("temperature_2m", 25)), latest["timestamp"].hour, latest["timestamp"].dayofweek, latest["timestamp"].month, zone_name)
